...
function obcin()
{
    $c = "SELECT NU_PASS FROM PAX WHERE NOM_PAX = 'Nombres según pasaporte' ORDER BY PAX.ZONA ASC";
    $xc = mysqli_query($_SESSION['uncons'], $c);
    $num = mysqli_num_rows($xc);
    for ($i = 0; $i < $num; $i ++) {
        $rxc[$i] = mysqli_fetch_array($xc, MYSQLI_NUM)[0];
    ...
    for ($i = 0; $i < count($rxc); $i ++) {
        $c = "SELECT PAX.NU_PASS, PAX.EMAIL_PAX, PAX.ZONA FROM PAX WHERE PAX.NU_PASS = '$rxc[$i]'";
        $aux = mysqli_query($_SESSION['uncons'], $c);
